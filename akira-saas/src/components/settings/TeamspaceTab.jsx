...
/*
 * Espacio de equipo (grupo AdministraciÃ³n) â€” gestiÃ³n de los espacios de equipo
 * dentro del workspace. Para un solo dueÃ±o es bÃ¡sico; se ampliarÃ¡ con equipos.
 */
  ...
  return (
    <div>
      <RowSection title="Espacios de equipo" description="Agrupa a las personas y su contenido en espacios dentro de tu workspace.">
        <Row title="Espacio de equipo predeterminado" description="Los miembros nuevos se unen automÃ¡ticamente a este espacio.">
          <span style={{ fontSize: '12.5px', fontWeight: 600, color: 'var(--text-2)' }}>{org ? org.name : 'Mi espacio'}</span>
        </Row>
        <Row title="Unir automÃ¡ticamente a los miembros nuevos" description="Cada persona nueva entra directamente al espacio predeterminado.">
          <Toggle checked={prefs.teamspace_default_join} onClick={toggle('teamspace_default_join')} />
        </Row>
        <Row title="Limitar la creaciÃ³n de espacios a los propietarios" description="Solo los administradores podrÃ¡n crear nuevos espacios de equipo." last>
      ...
      <RowSection title="Espacios existentes" description="Los espacios de equipo a los que tienes acceso.">
        <Row title={org ? org.name : 'Mi espacio'} description={(members ? members.length : 1) + ((members ? members.length : 1) === 1 ? ' miembro' : ' miembros') + ' Â· Predeterminado'} last>
