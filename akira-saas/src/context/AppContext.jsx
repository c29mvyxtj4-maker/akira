...
export function AppProvider({ children }) {
  ...
  const fetchAll = useCallback(function() {
    ...
    Promise.allSettled([
      ...
      supabase.from('projects').select('*, clients(id,name)').eq('archived', false).order('due_date', { ascending: true, nullsFirst: false }),
      supabase.from('services').select('*').eq('archived', false),
      supabase.from('subscriptions').select('*').eq('archived', false),
