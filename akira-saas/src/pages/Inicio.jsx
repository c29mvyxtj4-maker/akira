...
import { Home, MessageSquare, Calendar as CalendarIcon, Inbox, LayoutDashboard, Users, FolderKanban, Wallet, FileText, Clock, BookOpen, ChevronRight, TrendingUp } from 'lucide-react'
...
function fmtEur(n) { return fmtEuro(n) }
    ...
    <div style={{ height: '100%', position: 'relative', overflow: 'hidden', background: 'var(--bg-base)' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, height: '100%', overflowY: 'auto', paddingTop: 'calc(var(--safe-top) + 18px)' }}>
        <div style={{ maxWidth: '1080px', margin: '0 auto', padding: '0 24px calc(112px + var(--safe-bottom))' }}>
          ...
          {/* WIDGETS BUTTON - FIXED TOP RIGHT */}
          <div style={{ position: 'fixed', top: '20px', right: '20px', zIndex: 999 }}>
            <button onClick={() => setShowWidgets(!showWidgets)} style={{ padding: '10px 16px', borderRadius: '8px', background: '#e63946', color: 'white', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: 700, boxShadow: '0 4px 12px rgba(230,57,70,0.3)' }}>
          ...
          {showWidgets && dashboard && (
            <div style={{ marginBottom: '24px', marginTop: '80px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', overflowX: 'auto', paddingBottom: '4px', marginBottom: '22px' }}>
            <BorderGlow {...GLOW}>
              <button type="button" onClick={openMenu} aria-label="Tu cuenta" style={{ width: '38px', height: '38px', flexShrink: 0, borderRadius: '50%', border: 'none', background: 'var(--gradient-brand)', color: '#fff', fontSize: '14px', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: 'var(--shadow-brand)' }}>{initial}</button>
            </BorderGlow>
            {menuOpen && (<AccountMenu anchor={menuAnchor} orgName={org ? org.name : null} plan={org && org.plan ? org.plan : 'gratuito'} memberCount={members ? members.length : 1} initial={initial} email={user ? user.email : ''} workspaces={workspaces || []} activeOrgId={org ? org.id : null} onSwitch={function(id) { setMenuOpen(false); switchWorkspace(id) }} onCreateWorkspace={handleCreateWorkspace} onSettings={function() { openSettings('profile') }} onInvite={function() { openSettings('team') }} onSignOut={function() { setMenuOpen(false); signOut() }} onClose={function() { setMenuOpen(false) }} />)}
            ...
            {PILLS.map(function(p) {
              ...
              return (
                <BorderGlow key={p.label} {...GLOW}>
                  <button type="button" onClick={function() { navigate(p.to) }} title={p.label} aria-label={p.label} style={{ position: 'relative', width: '40px', height: '38px', borderRadius: '999px', flexShrink: 0, background: 'var(--bg-3)', border: 'none', color: 'var(--text-3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon style={{ width: '16px', height: '16px' }} />
                    {showBadge && (<span style={{ position: 'absolute', top: '4px', right: '5px', minWidth: '15px', height: '15px', padding: '0 4px', borderRadius: '999px', background: 'var(--brand)', color: '#fff', fontSize: '9px', fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 0 2px var(--bg-base)' }}>{unread > 9 ? '9+' : unread}</span>)}
                  </button>
                </BorderGlow>
              )
            })}
          ...
          {/* Saludo */}
          <motion.h1 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: DUR.slow, ease: EASE.out }} style={{ fontSize: '28px', fontWeight: 900, color: 'var(--text-1)', letterSpacing: '-0.03em' }}>
            Hola, {name}
          </motion.h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3" style={{ marginTop: '20px' }}>
            {KPIS.map(function(k, i) {
              var Icon = k.icon
              return (
                <motion.div key={k.label} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04, duration: DUR.slow, ease: EASE.out }} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '16px', position: 'relative', overflow: 'hidden' }}>
          ...
          {fin && fin.sparkline && (
            <>
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: DUR.slow, ease: EASE.out }} style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '18px' }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {QUICK.map(function(q) {
              var Icon = q.icon
              return (
                <motion.button key={q.label} type="button" onClick={function() { navigate(q.to) }} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }} transition={SPRING.snappy} onMouseEnter={function(e) { e.currentTarget.style.borderColor = 'var(--brand-border)'; e.currentTarget.style.background = 'var(--brand-dim)' }} onMouseLeave={function(e) { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'var(--bg-2)' }} style={{ textAlign: 'left', background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '16px', cursor: 'pointer', transition: 'background 0.15s, border-color 0.15s' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
            {SECTIONS.map(function(s) {
              var Icon = s.icon
              return (
                <button key={s.label} type="button" onClick={function() { navigate(s.to) }} style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%', textAlign: 'left', background: 'transparent', border: '1px solid transparent', borderRadius: 'var(--radius-md)', padding: '11px 12px', cursor: 'pointer', color: 'var(--text-2)', transition: 'background 0.12s, border-color 0.12s' }} onMouseEnter={function(e) { e.currentTarget.style.background = 'var(--bg-2)'; e.currentTarget.style.borderColor = 'var(--border)' }} onMouseLeave={function(e) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'transparent' }}>
