...
export default function Mensajes() {
  ...
  function nameOf(uid) { var p = team[uid]; return (p && p.full_name) || (uid === me ? 'TÃº' : 'Miembro') }
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg-base)', paddingTop: 'calc(var(--safe-top) + 14px)' }}>
      <div style={{ maxWidth: '960px', width: '100%', margin: '0 auto', padding: '0 24px', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        {!orgId ? (
          <p style={{ fontSize: '14px', color: 'var(--text-4)' }}>Cargando tu organizaciÃ³nâ€¦</p>
        ) : tab === 'chat' ? (
          <>
            <div style={{ flex: 1, overflowY: 'auto', paddingBottom: '10px', display: 'flex', flexDirection: 'column', gap: '10px', minHeight: 0 }}>
              {messages.length === 0 ? (
                <p style={{ fontSize: '13px', color: 'var(--text-5)', textAlign: 'center', paddingTop: '32px' }}>AÃºn no hay mensajes. Escribe el primero.</p>
                ...
                return (
                  <div key={m.id} style={{ display: 'flex', gap: '8px', flexDirection: mine ? 'row-reverse' : 'row' }}>
                    ...
                    <div style={{ maxWidth: '78%' }}>
                      ...
                      <p style={{ fontSize: '10px', color: 'var(--text-5)', marginTop: '2px', textAlign: mine ? 'right' : 'left' }}>{mine ? '' : nameOf(m.user_id) + ' Â· '}{fmtTime(m.created_at)}</p>
                    </div>
                  </div>
                )
              })}
            ...
            <div style={{ display: 'flex', gap: '8px', padding: '10px 0 calc(var(--safe-bottom) + 12px)', flexShrink: 0 }}>
              <input value={draft} onChange={function (e) { setDraft(e.target.value) }}
                ...
                placeholder="Escribe un mensajeâ€¦" className="input-base" style={{ flex: 1, height: '42px' }} />
          ...
          <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 'calc(var(--safe-bottom) + 24px)', display: 'flex', flexDirection: 'column', gap: '12px', minHeight: 0 }}>
            {isOwner && (
              <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <input value={annTitle} onChange={function (e) { setAnnTitle(e.target.value) }} placeholder="TÃ­tulo (opcional)" className="input-base" style={{ height: '38px' }} />
                <textarea value={annBody} onChange={function (e) { setAnnBody(e.target.value) }} placeholder="Escribe el anuncio para tu equipoâ€¦" rows={3} className="input-base" style={{ resize: 'vertical' }} />
            ...
            {anns.length === 0 ? (
              <p style={{ fontSize: '13px', color: 'var(--text-5)', textAlign: 'center', paddingTop: '24px' }}>No hay anuncios todavÃ­a.{isOwner ? '' : ' AquÃ­ verÃ¡s lo que publique tu responsable.'}</p>
            ) : anns.map(function (a) {
              return (
                <div key={a.id} style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '12px 14px' }}>
                  ...
                  <p style={{ fontSize: '11px', color: 'var(--text-5)', marginTop: '8px' }}>{nameOf(a.author_id)} Â· {fmtDate(a.created_at)}</p>
                </div>
              )
            })}
          </div>
