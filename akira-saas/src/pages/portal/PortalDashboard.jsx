...
export default function PortalDashboard() {
  ...
  if (loading) {
    return (
      <div style={Object.assign({ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-base)' }, brandVars)}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: 'var(--gradient-brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', fontWeight: 900, color: '#fff', margin: '0 auto 16px', boxShadow: '0 0 24px rgba(230,57,70,0.3)' }}>A</div>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-base)', padding: '20px' }}>
        <div style={{ textAlign: 'center', maxWidth: '320px' }}>
          ...
          <button type="button" onClick={handleSignOut}
            style={{ padding: '8px 20px', borderRadius: '8px', background: 'var(--gradient-brand)', border: 'none', color: '#fff', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}
  ...
  return (
    <div style={Object.assign({ minHeight: '100vh', background: 'var(--bg-base)', display: 'flex', flexDirection: 'column' }, brandVars)}>
      {/* Header */}
      <div style={{ background: 'var(--bg-1)', borderBottom: '1px solid var(--border)', padding: '0 20px', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {/* ← NUEVO: logo de la agencia si existe, si no la inicial con su color */}
          {branding.logo_url ? (
      ...
      <div style={{ padding: '20px 20px 0', background: 'var(--bg-1)', borderBottom: '1px solid var(--border)' }}>
        <p style={{ fontSize: '18px', fontWeight: 800, color: 'var(--text-1)', marginBottom: '14px', letterSpacing: '-0.02em' }}>
          Hola, {data && data.portalUser && (data.portalUser.name || user && user.email)} 👋
        </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 200px)' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', paddingTop: '12px', borderTop: '1px solid var(--border)' }}>
              <input
                ...
                onKeyDown={function(e) { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage() } }}
                placeholder="Escribe un mensaje..."
                style={{ flex: 1, background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--text-1)', borderRadius: '10px', padding: '10px 14px', fontSize: '14px', fontFamily: 'inherit', outline: 'none' }}
              />
              <button type="button" onClick={handleSendMessage} disabled={!msgInput.trim() || sending}
                style={{ width: '40px', height: '40px', borderRadius: '10px', background: msgInput.trim() && !sending ? 'var(--gradient-brand)' : 'var(--bg-4)', border: 'none', color: msgInput.trim() && !sending ? '#fff' : 'var(--text-5)', cursor: msgInput.trim() && !sending ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.1s' }}
              >
                  ...
                  >
                    ...
                    {isPending && (
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button type="button"
                          onClick={function() { handleApprovalAction(a.id, 'revision', window.prompt('Escribe tu feedback o comentarios:') || '') }}
                        ...
                        <button type="button"
                          onClick={function() { handleApprovalAction(a.id, 'rejected', window.prompt('Motivo del rechazo:') || '') }}
                        ...
                        >Rechazar</button>
                        <button type="button"
                          onClick={function() { handleApprovalAction(a.id, 'approved', '') }}
