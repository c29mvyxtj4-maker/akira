...
export async function updateInvoice(id, form) {
  var totals = calcTotals(form.items, form.tax_rate)
  var res = await supabase.from('invoices').update({
    client_id:  form.client_id || null,
...
export async function updateInvoiceStatus(id, status) {
  var res = await supabase.from('invoices').update({ status: status }).eq('id', id).select('*, clients(id, name, company)').single()
  if (res.error) throw res.error
  return res.data
}

export async function archiveInvoice(id) {
  var res = await supabase.from('invoices').update({ archived: true }).eq('id', id)
  if (res.error) throw res.error
  return true
}