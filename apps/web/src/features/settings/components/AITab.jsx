...
/*
 * IA de AKIRA (grupo Funciones) â€” equivalente a "IA de Notion": personalizaciÃ³n
 * del asistente, instrucciones, habilidades y modelo. Persistido en localStorage.
 */
var SKILLS = [
  { key: 'skill_improve', icon: Wand2,        label: 'Mejorar redacciÃ³n', desc: 'Reescribe un texto para que sea mÃ¡s claro y profesional.' },
  { key: 'skill_fix',     icon: CheckCircle2, label: 'Corregir',          desc: 'Corrige ortografÃ­a y gramÃ¡tica manteniendo el tono.' },
...
function AITab() {
  ...
  return (
    <div>
      <RowSection title="PersonalizaciÃ³n" description="Elige cÃ³mo se llama y se comporta tu asistente de IA.">
        <Row title="Nombre del asistente" description="El nombre que verÃ¡s al abrir Â«Preguntar a AKIRAÂ».">
        ...
        <Row title="Modelo" description="Modelo de lenguaje que impulsa las respuestas del asistente." last>
          <RowSelect value={prefs.ai_model} onChange={function (e) { setPref('ai_model', e.target.value) }}
            options={[
              { value: 'gemini-1.5-flash', label: 'Gemini 1.5 Flash Â· rÃ¡pido' },
              { value: 'gemini-1.5-pro',   label: 'Gemini 1.5 Pro Â· potente' },
              { value: 'gemini-2.0-flash', label: 'Gemini 2.0 Flash' },
            ]} />
        </Row>
      </RowSection>

      <RowSection title="Instrucciones" description="Contexto permanente que AKIRA tendrÃ¡ en cuenta en cada conversaciÃ³n.">
        <div style={{ padding: '14px 0' }}>
          <textarea value={instructions} onChange={function (e) { setInstructions(e.target.value) }}
            rows={4} placeholder="Ej: Soy diseÃ±ador freelance. Responde en espaÃ±ol, tono directo, y cuando hable de facturas usa euros."
      ...
      <RowSection title="Habilidades" description="Acciones rÃ¡pidas que AKIRA puede ejecutar sobre el texto seleccionado.">
      ...
      <RowSection title="Comportamiento">
        <Row title="Sugerencias proactivas" description="Permite que AKIRA proponga acciones (crear tareas, facturasâ€¦) a partir de tu contexto." last>
